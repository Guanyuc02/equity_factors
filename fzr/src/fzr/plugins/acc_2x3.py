...
def compute(ctx: FactorContext, *, start: str, end: str) -> pd.DataFrame:
    """Sloan (1996) working capital accruals factor from 2×3 sorts.

    Characteristic:
        acc = (ib − oancf) / ((at + lag(at)) / 2) measured at CCM-linked Compustat anchors aligned to June.
    Construction:
        ...
        - Portfolios: size (S/B) × accruals (L/M/H).
        - Factor = WL − WH (low accrual minus high accrual).
    """
    ...
    required_cols = {"ib", "oancf", "at"}
    missing = sorted(required_cols - set(a.columns))
    if missing:
        raise ValueError(f"ccm_linked_funda missing required columns for acc: {missing}")
    ...
    a = a.sort_values(["permno", "ym"])
    a["lag_at"] = a.groupby("permno", group_keys=False)["at"].shift(1)
    avg_assets = (a["at"] + a["lag_at"]) / 2.0
    accrual_num = a["ib"] - a["oancf"]
    valid = avg_assets.notna() & avg_assets.ne(0.0) & accrual_num.notna()
    a["acc"] = np.nan
    a.loc[valid, "acc"] = (accrual_num.loc[valid] / avg_assets.loc[valid]).to_numpy()
    ...
    acc_bp = nyse_quantiles(
        june,
        value_col="acc",
        date_col="jdate",
        exch_col="primaryexch",
        sample_filter=(june["me"].gt(0) & june["acc"].notna()),
    )
    acc_bp = acc_bp.rename(columns={"acc30": "acc30", "acc70": "acc70"})

    june_bp = june.merge(sz_bp, on="jdate", how="left").merge(acc_bp, on="jdate", how="left")
    ...
    june_ports = assign_2x3(
        ...
        out_size="szport",
        out_char="accport",
        valid_mask=(june_bp["me"].gt(0) & june_bp["acc"].notna()),
    )

    crsp_monthly = crsp3[["mthcaldt", "permno", "mthret", "wt", "ffyear", "jdate"]].copy()
    lab = june_ports[["permno", "jdate", "szport", "accport"]].copy()
    ...
    d = monthly_labeled[
        ...
        & monthly_labeled[["szport", "accport"]].ne("").all(axis=1)
    ]
    ...
    factor = (
        wide[["jdate", "SL", "SM", "SH", "BL", "BM", "BH"]]
            .assign(
                ...
                acc=lambda x: apply_orientation(x["WH"], x["WL"], "low_minus_high"),
            )[["jdate", "acc"]]

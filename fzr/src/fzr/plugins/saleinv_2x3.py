...
def compute(ctx: FactorContext, *, start: str, end: str) -> pd.DataFrame:
    """Sales-to-inventory (saleinv = sale / invt) factor from 2×3 sorts.

    Characteristic:
        saleinv = sale / invt measured at CCM-linked Compustat anchors aligned to each CRSP June.
    Construction:
        ...
        - Portfolios: size (S/B) × saleinv (L/M/H).
        - Factor sign matches Ou–Penman convention: WH − WL (high saleinv minus low saleinv).
    """
    ...
    if missing:
        raise ValueError(f"ccm_linked_funda missing required columns for saleinv: {missing}")
    ...
    si_bp = nyse_quantiles(
        ...
        date_col="jdate",
        exch_col="primaryexch",
        sample_filter=(june["me"].gt(0) & june["saleinv"].notna()),
    )
    ...
    june_ports = assign_2x3(
        ...
        char_q30_col="saleinv30",
        char_q70_col="saleinv70",
        out_size="szport",
        out_char="saleinvport",
        valid_mask=(june_bp["me"].gt(0) & june_bp["saleinv"].notna()),
    )

    crsp_monthly = crsp3[["mthcaldt", "permno", "mthret", "wt", "ffyear", "jdate"]].copy()
    lab = june_ports[["permno", "jdate", "szport", "saleinvport"]].copy()
    lab["ffyear"] = lab["jdate"].dt.year
    monthly_labeled = crsp_monthly.merge(lab.drop(columns=["jdate"]), on=["permno", "ffyear"], how="left")

    d = monthly_labeled[(monthly_labeled["wt"] > 0) & monthly_labeled[["szport", "saleinvport"]].ne("").all(axis=1)]
    ...
    factor = (
        wide[["jdate", "SL", "SM", "SH", "BL", "BM", "BH"]]
            .assign(
                ...
                saleinv=lambda x: apply_orientation(x["WH"], x["WL"], "high_minus_low"),
            )[["jdate", "saleinv"]]

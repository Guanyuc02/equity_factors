...
def compute(ctx: FactorContext, *, start: str, end: str) -> pd.DataFrame:
    """R&D-to-sales (rds) factor via 2×3 sorts.

    Characteristic:
        rds = xrd / sale using fiscal-year data aligned to June.
    Factor sign: WH − WL.
    """
    june, crsp3, month_index = _prep_june_funda(
        ...
        required_cols=["xrd", "sale"],
        warm_months=13,
        require_be=True,
    )

    denom = pd.to_numeric(june.get("sale"), errors="coerce")
    eps = 1e-6
    valid = denom.abs() > eps
    june["rds_char"] = np.where(valid, june["xrd"] / denom, np.nan)

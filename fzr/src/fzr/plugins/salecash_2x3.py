...
def compute(ctx: FactorContext, *, start: str, end: str) -> pd.DataFrame:
    """Ou–Penman sales-to-cash (sale / che) factor from 2×3 sorts.

    Characteristic:
        salecash = sale / che using CCM-linked Compustat anchors aligned to each CRSP June.
    Construction:
        - Filter to firms with positive BE and non-financial SIC.
        - Size breakpoints: NYSE median; characteristic breakpoints: NYSE 30/70 quantiles.
        - Portfolios: size (S/B) × salecash (L/M/H).
        - Factor = WH − WL (high salecash minus low salecash).
    """
    ...
    required_cols = {"sale", "che"}
    missing = sorted(required_cols - set(a.columns))
    if missing:
        raise ValueError(f"ccm_linked_funda missing required columns for salecash: {missing}")
    ...
    denom = a["che"].astype("float64")
    valid = denom.notna() & denom.ne(0.0)
    a.loc[valid, "salecash"] = (num.loc[valid] / denom.loc[valid]).to_numpy()
    ...
    sc_bp = nyse_quantiles(
        june,
        value_col="salecash",
        date_col="jdate",
        exch_col="primaryexch",
        sample_filter=(june["me"].gt(0) & june["salecash"].notna()),
    )
    sc_bp = sc_bp.rename(columns={"salecash30": "salecash30", "salecash70": "salecash70"})

    june_bp = june.merge(sz_bp, on="jdate", how="left").merge(sc_bp, on="jdate", how="left")
    ...
    june_ports = assign_2x3(
        ...
        char_q30_col="salecash30",
        char_q70_col="salecash70",
        out_size="szport",
        out_char="scport",
        valid_mask=(june_bp["me"].gt(0) & june_bp["salecash"].notna()),
    )

    crsp_monthly = crsp3[["mthcaldt", "permno", "mthret", "wt", "ffyear", "jdate"]].copy()
    lab = june_ports[["permno", "jdate", "szport", "scport"]].copy()
    lab["ffyear"] = lab["jdate"].dt.year
    monthly_labeled = crsp_monthly.merge(lab.drop(columns=["jdate"]), on=["permno", "ffyear"], how="left")

    d = monthly_labeled[(monthly_labeled["wt"] > 0) & monthly_labeled[["szport", "scport"]].ne("").all(axis=1)]
    ...
    factor = (
        wide[["jdate", "SL", "SM", "SH", "BL", "BM", "BH"]]
            .assign(
                ...
                salecash=lambda x: apply_orientation(x["WH"], x["WL"], "high_minus_low"),
            )[["jdate", "salecash"]]

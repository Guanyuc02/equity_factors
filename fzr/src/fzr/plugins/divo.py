...
def compute(ctx: FactorContext, *, start: str, end: str) -> pd.DataFrame:
    ...
    f["is_event"] = (f["pay_div"] == 0) & (f["pay_div_lag"] == 1)

    char = (
        f.groupby(["permno", "ym"], as_index=False)["is_event"]
        .max()
        .rename(columns={"is_event": "divo_char"})
    )
    char["divo_char"] = char["divo_char"].astype(float)
    char["ffyear"] = pd.to_datetime(char["ym"]).dt.year

    char = char[["permno", "ffyear", "divo_char"]]
    d = d.merge(char, on=["permno", "ffyear"], how="left")
    d["divo_char"] = d["divo_char"].fillna(0.0)

    mask = (d["wt"] > 0) & d["mthret"].notna()
    panel = d.loc[mask, ["mthcaldt", "mthret", "wt", "divo_char"]].copy()
